...
const DEFAULT_BALL_AVOID_MM: f32 = 1_500.0;
const DEFAULT_PENALTY_MARGIN_MM: f32 = 250.0;
...
pub fn drive_to_target(
  cfg: &Config, cp_data: &CpRobot, robot_self: CpTrackedRobot, target: CpVector2, options: OrcaOptions,
) -> OrcaPlan {
  let self_pos = cp_to_vec2(robot_self.pos);
  let self_vel = robot_velocity(robot_self);
  let mut target = cp_to_vec2(target);
  ...
  let _accel_preview = accel_limit(current_speed_mm_s, desired_speed, options.accel_mm_s2, DEFAULT_DT_S);
...
#[allow(dead_code)]
#[inline]
pub fn accel_limit(current_speed_mm_s: f32, target_speed_mm_s: f32, accel_mm_s2: f32, dt_s: f32) -> f32 {
...
fn avoid_dynamic_robots(
  cfg: &Config, cp_data: &CpRobot, robot_self: CpTrackedRobot, velocity: Vector2, options: OrcaOptions,
) -> Vector2 {
  let self_pos = cp_to_vec2(robot_self.pos);
  let self_vel = robot_velocity(robot_self);
  ...
  for robot in opponents {
    ...
    let obstacle_pos = cp_to_vec2(robot.pos);
...
#[inline]
fn avoid_ball(cp_data: &CpRobot, robot_self: CpTrackedRobot, velocity: Vector2, options: OrcaOptions) -> Vector2 {
  let self_pos = cp_to_vec2(robot_self.pos);
  let self_vel = robot_velocity(robot_self);
...
#[inline]
fn avoid_penalty_areas(cfg: &Config, robot_self: CpTrackedRobot, velocity: Vector2, options: OrcaOptions) -> Vector2 {
  let self_pos = cp_to_vec2(robot_self.pos);
  let mut out = velocity;

  for rect in penalty_rects(cfg).into_iter() {
	let expanded = expand_rect(rect, options.penalty_margin_mm);
	out = steer_around_rect(out, self_pos, expanded, options.static_influence_mm, options.max_speed_mm_s);
  }
...
#[inline]
fn project_target_outside_penalty_areas(cfg: &Config, target: Vector2, margin_mm: f32) -> Vector2 {
  let mut out = target;
...
fn steer_around_dynamic(
  velocity: Vector2, self_pos: Vector2, self_vel: Vector2, obstacle_pos: Vector2, obstacle_vel: Vector2,
  ...
  if closest_dist < influence_mm || current_dist < threshold {
    ...
    let lateral = normalize(vec2(-rel_pos.y, rel_pos.x));
...
#[inline]
fn steer_around_rect(velocity: Vector2, self_pos: Vector2, rect: Rect, influence_mm: f32, max_speed_mm_s: f32) -> Vector2 {
...
#[inline]
fn rect_escape_direction(point: Vector2, rect: Rect) -> Vector2 {
...
#[inline]
fn project_outside_rect(point: Vector2, rect: Rect, margin_mm: f32) -> Vector2 {
  let mut out = point;
...
#[inline]
fn point_in_rect(point: Vector2, rect: Rect) -> bool {
  point.x >= rect.min_x && point.x <= rect.max_x && point.y >= rect.min_y && point.y <= rect.max_y
}

#[inline]
fn clamp_point(point: Vector2, rect: Rect) -> Vector2 {
  vec2(point.x.clamp(rect.min_x, rect.max_x), point.y.clamp(rect.min_y, rect.max_y))
}
...
#[inline]
fn robot_velocity(robot: CpTrackedRobot) -> Vector2 {
  robot.vel.map_or(vec2(0.0, 0.0), cp_to_vec2)
}
