...
use crate::communication::{communication_receiver, send_flags, EventShare, TeensyOut};
    ...
    pub async fn run(&mut self) {
        ...
        loop {
            ...
            if self.config.robot_team.as_str() == "yellow" {
                self.packets.robot_self = *self.packets.cp_data
                    .robots_yellow
                    .iter()
            ...
            match CpState::try_from(self.packets.cp_data.cmd.state).unwrap_or(CpState::StateUnspecified) {
                ...
                CpState::StateHalt => {
                    ...
                    self.packets.robot_msg = nav_command_to_teensy(mem::take(&mut self.packets.robot_msg), self.orca.latest());
                }
                CpState::StateStop => {
            ...
            if inside_field(&self.config, Vec2f::new_from_cp(self.packets.robot_self.pos)) || self.packets.robot_self.visibility <= 20 {
